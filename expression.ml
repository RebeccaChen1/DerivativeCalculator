...
(* test whether an expression e contains a variable *)
let rec contains_var (e : expression) : bool =
    match e with
    | Var -> true
    | Unop (_a, b) -> contains_var b
    | Num _x -> false
    | Binop (_bi, ex1, ex2) -> contains_var ex1 || contains_var ex2 ;;
...
(* evaluates an expression for a value of x *)
let rec evaluate (e : expression) (x : float) : float =
  match e with
  | Unop (up, ex) -> (match up with
                     | Sin -> sin (evaluate ex x)
                     | Cos -> cos (evaluate ex x) 
                     | Ln -> log (evaluate ex x)
                     | Neg -> -. (evaluate ex x))
  | Binop (bi, ex1, ex2) -> (match bi with
                            | Add -> evaluate ex1 x +. evaluate ex2 x 
                            | Sub -> evaluate ex1 x -. evaluate ex2 x
                            | Mul -> evaluate ex1 x *. evaluate ex2 x
                            | Div -> evaluate ex1 x /. evaluate ex2 x
                            | Pow -> evaluate ex1 x ** evaluate ex2 x)
  | Num a -> a
  | Var -> x
...
let rec find_zero (e: expression) 
                  (g: float) 
                  (epsilon: float) 
                  (lim: int)
	              : float option =
  (* calculates final guess after lim tries *)
  let guess = evaluate (Binop (Sub, Num g, Binop (Div, e, derivative e))) g in
    if lim = 0
    then (if abs_float (evaluate e guess) < epsilon then Some guess else None)
    else find_zero e guess epsilon (lim - 1) ;;
...
let minutes_spent_on_part2 () : int = 300 ;;
